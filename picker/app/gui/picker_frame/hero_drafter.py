...
from picker.app.gui.picker_frame.hero_drafter_button import HeroDrafterButton
...
class HeroDrafter(AbstractFrame):
    def __init__(self, runner: DesktopProgramRunner, master=None, *args, **kwargs):
        ...
        for hero in all_heroes:
            ...
            # Bind mouse wheel to each hero button
            rect.bind("<MouseWheel>", self.on_mousewheel)
    ...
    def update_ui(self):
        for name, rect in self.rectangles.items():
            if not self.text or name.lower().startswith(self.text.lower()):
                ...
                # Ensure mouse wheel is bound to visible heroes
                rect.bind("<MouseWheel>", self.on_mousewheel)
            else:
                rect.pack_forget()
